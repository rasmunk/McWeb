...
if [ "$OLD" == $NEW ]; then
	...
	echo
	echo claiming ownership of all instrument files > /srv/mcweb/McWeb/mcsimrunner/static/compile_status.html
	chown -R www-data:www-data sim/ >> /srv/mcweb/McWeb/mcsimrunner/static/compile_status.html
